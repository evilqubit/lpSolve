...
function createKidLabel ($data){
	$output = "<div class='student-entry label label-primary ui-widget-header draggable' data-name='".strtolower($data['name'])."' data-id='$data[id]'>$data[name] #$data[id]";
			...
			<div class="col-md-12">
			 ...
			 <p>Drag & Drop kids into the calendar and Recalculate the schedule!</p>
				...
				<form id="hidden_form" method="POST" action="">
					...
					<button class="btn btn-success hide_me" type="submit" id="toggle_students">Show Students</button>
						...
						<tbody>
							...
							for ( $i = 1; $i <= $home_data->slot_total_count; $i++){
								...
								<tr>
									...
									for ( $y = 1; $y <= $home_data->days_a_week; $y++){
										$slot_key = 'x'.$y.'y'.$i;
										...
										if ( isset ($home_data->students_schedule_xn_format[$slot_key]) && !empty($home_data->students_schedule_xn_format[$slot_key]['Students']) ){
											...
											foreach ($home_data->students_schedule_xn_format[$slot_key]['Students'] as $student_data)
											{
												...
												$student_entries .= createKidLabel ($student_data, 'return');
											...
											?>
											<td data-slot="<?php echo $slot_key;?>" data-ids="<?php echo $csv_ids;?>" class="col-md-1 ui-widget-header droppable">
												<div class="student_count"><?php echo $slot_student_count;?> Students</div>
